...
function RageModule.new(config)
	...
	-- ===== SAFE WALLBANG =====
	self.SAFE_WALLBANG_ENABLED = false
...
function RageModule:IsPartVisible(targetPart, targetChar)
	...
	-- Check if player is near a wall/corner (auto peek detection)
	...
	for _, checkDir in ipairs(checkDirections) do
		local wallCheck = self.Workspace:Raycast(myHrp.Position, checkDir * 3, wallCheckParams)
		if wallCheck and wallCheck.Distance < 3 then
			isNearWall = true
			break
		end
	end
	...
	-- NO TP WALLBANG: Infinite penetration (shoot through everything)
	...
	-- ===== SAFE WALLBANG vs NORMAL WALLBANG =====
	if self.SAFE_WALLBANG_ENABLED then
		return self:_safeWallbangCheck(curOrigin, targetPos, unit, targetChar, isNearWall)
	else
		return self:_normalWallbangCheck(curOrigin, targetPos, unit, targetChar, isNearWall)
	end
end

-- ============================================
-- NORMAL WALLBANG (original full-power)
-- ============================================
function RageModule:_normalWallbangCheck(curOrigin, targetPos, unit, targetChar, isNearWall)
	local penetrationCount = 0
	
	local maxPenetrations
	if self.RAPIDFIRE_ENABLED then
		maxPenetrations = isNearWall and 8 or 5
	else
		maxPenetrations = isNearWall and 6 or 3.5
	end
	...
	for _ = 1, 15 do
		...
		if hit then
			...
			local penetrationCost = 1
			
			local costMultiplier = self.RAPIDFIRE_ENABLED and 0.7 or 0.9
			
			if material == Enum.Material.Wood or 
			   material == Enum.Material.WoodPlanks or
			   material == Enum.Material.Fabric or
			   material == Enum.Material.Cardboard or
			   name:find("wood") or name:find("box") or name:find("crate") then
				isPenetrable = true
				penetrationCost = (isNearWall and 0.25 or 0.4) * costMultiplier
			
			elseif material == Enum.Material.Plastic or
			       material == Enum.Material.Glass or
			       material == Enum.Material.Ice or
			       material == Enum.Material.Foil or
			       material == Enum.Material.Brick or
			       name:find("glass") or name:find("window") or name:find("door") then
				isPenetrable = true
				penetrationCost = (isNearWall and 0.5 or 0.8) * costMultiplier
			
			elseif material == Enum.Material.Concrete or
			       material == Enum.Material.Cobblestone or
			       material == Enum.Material.Slate or
			       name:find("wall") or name:find("concrete") then
				isPenetrable = true
				penetrationCost = (isNearWall and 0.8 or 1.2) * costMultiplier
			
			elseif material == Enum.Material.Metal or
			       material == Enum.Material.DiamondPlate or
			       material == Enum.Material.CorrodedMetal or
			       name:find("metal") or name:find("steel") or name:find("iron") then
				isPenetrable = true
				penetrationCost = (isNearWall and 1.2 or 1.8) * costMultiplier
			
			elseif material == Enum.Material.Granite or
			       material == Enum.Material.Marble then
				isPenetrable = true
				penetrationCost = (isNearWall and 1.5 or 2.2) * costMultiplier
			
			else
				local thickness = hit.Size.Magnitude / 3
				if thickness < 3 then
					isPenetrable = true
					penetrationCost = (isNearWall and 0.3 or 0.5) * costMultiplier
				end
			end
			...
			if isPenetrable and penetrationCount + penetrationCost <= maxPenetrations then
				penetrationCount = penetrationCount + penetrationCost
				curOrigin = res.Position + unit * 0.2
				continue
			end
		end
...
-- ============================================
-- SAFE WALLBANG (nerfed penetration)
-- ============================================
function RageModule:_safeWallbangCheck(curOrigin, targetPos, unit, targetChar, isNearWall)
	local penetrationCount = 0
	
	-- Nerfed: much lower max penetration, no wall bonus
	local maxPenetrations
	if self.RAPIDFIRE_ENABLED then
		maxPenetrations = 2.0 -- was 5-8
	else
		maxPenetrations = 1.5 -- was 3.5-6
	end

	for _ = 1, 8 do -- fewer iterations (was 15)
		local res = self.Workspace:Raycast(curOrigin, targetPos - curOrigin, self._rayParams)
		...
		if hit then
			...
			-- Soft/transparent objects still pass through freely
			if isHamik or isSoft then
				curOrigin = res.Position + unit * 0.2
				continue
			end
			...
			-- No rapidfire discount, no nearWall bonus in safe mode
			local costMultiplier = 1.0
			
			-- Only light materials are penetrable
			if material == Enum.Material.Wood or 
			   material == Enum.Material.WoodPlanks or
			   material == Enum.Material.Fabric or
			   material == Enum.Material.Cardboard or
			   name:find("wood") or name:find("box") or name:find("crate") then
				isPenetrable = true
				penetrationCost = 0.7 * costMultiplier -- was 0.25-0.4
			...
			elseif material == Enum.Material.Plastic or
			       ...
			       name:find("glass") or name:find("window") or name:find("door") then
				isPenetrable = true
				penetrationCost = 1.0 * costMultiplier -- was 0.5-0.8
			
			elseif material == Enum.Material.Brick or
			       material == Enum.Material.Concrete or
			       material == Enum.Material.Cobblestone or
			       material == Enum.Material.Slate or
			       name:find("wall") or name:find("concrete") then
				isPenetrable = true
				penetrationCost = 1.5 * costMultiplier -- was 0.8-1.2
			
			-- Metal: NOT penetrable in safe mode
			elseif material == Enum.Material.Metal or
			       material == Enum.Material.DiamondPlate or
			       material == Enum.Material.CorrodedMetal or
			       name:find("metal") or name:find("steel") or name:find("iron") then
				isPenetrable = false -- BLOCKED
			
			-- Granite/Marble: NOT penetrable in safe mode
			elseif material == Enum.Material.Granite or
			       material == Enum.Material.Marble then
				isPenetrable = false -- BLOCKED
			
			else
				-- Default: only very thin objects
				local thickness = hit.Size.Magnitude / 3
				if thickness < 1.5 then -- was 3
					isPenetrable = true
					penetrationCost = 0.8 * costMultiplier -- was 0.3-0.5
				end
			end
...
-- ============================================
-- SAFE WALLBANG SETTER
-- ============================================
function RageModule:SetSafeWallbangEnabled(value)
	self.SAFE_WALLBANG_ENABLED = value
	if value then
		self.Notification:Notify({
			Title = "Safe Wallbang",
			Content = "Nerfed penetration — only light materials",
			Icon = "shield",
			Duration = 3
		})
	else
		self.Notification:Notify({
			Title = "Safe Wallbang",
			Content = "Disabled — full penetration restored",
			Icon = "info",
			Duration = 3
		})
	end
...
function RageModule:GetSettings()
	return {
		...
		SAFE_WALLBANG_ENABLED = self.SAFE_WALLBANG_ENABLED,
	}
end
...
function RageModule:ApplySettings(settings)
	...
	if settings.SAFE_WALLBANG_ENABLED ~= nil then self:SetSafeWallbangEnabled(settings.SAFE_WALLBANG_ENABLED) end
end
